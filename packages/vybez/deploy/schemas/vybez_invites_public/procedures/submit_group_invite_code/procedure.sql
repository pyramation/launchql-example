-- Deploy: schemas/vybez_invites_public/procedures/submit_group_invite_code/procedure to pg
...
CREATE FUNCTION "vybez_invites_public".submit_group_invite_code (
  token text
)
  RETURNS boolean
  AS $$
DECLARE
  ...
  v_invite "vybez_invites_public".group_invites;
BEGIN
  ...
  SELECT * FROM "vybez_invites_public".group_invites i
  ...
  IF (v_email.email IS NOT NULL OR v_invite.multiple IS FALSE) THEN 
    UPDATE "vybez_invites_public".group_invites 
  ...
  WHERE id = v_invite.id;
  INSERT INTO "vybez_invites_public".group_claimed_invites 
    (sender_id, receiver_id, data, entity_id)
  VALUES (v_invite.sender_id, v_user.id, v_invite.data, v_invite.entity_id);
  IF (
    "vybez_memberships_private".group_memberships_perm_check(
        'invites_approved',
        v_invite.entity_id,
        v_invite.sender_id
    ) IS TRUE
   ) THEN 
    INSERT INTO "vybez_memberships_public".group_memberships 
...
VOLATILE
SECURITY DEFINER;
GRANT EXECUTE ON FUNCTION "vybez_invites_public".submit_group_invite_code TO authenticated;
COMMIT;
