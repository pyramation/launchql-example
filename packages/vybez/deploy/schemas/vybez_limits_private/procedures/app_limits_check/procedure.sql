-- Deploy: schemas/vybez_limits_private/procedures/app_limits_check/procedure to pg
...
CREATE FUNCTION "vybez_limits_private".app_limits_check (limitname citext, amount int default 1, user_id uuid default jwt_public.current_user_id())
  RETURNS BOOLEAN
AS $CODEZ$
DECLARE
...
BEGIN
    SELECT EXISTS (SELECT 1 FROM "vybez_limits_public".app_limits 
    ...
    END IF;
    SELECT * FROM "vybez_limits_public".app_limits
        WHERE name = limitname
        AND actor_id = user_id
    INTO rec;
