-- Deploy: schemas/vybez_memberships_private/trigger_fns/memberships_itg to pg
...
CREATE FUNCTION "vybez_memberships_private".memberships_itg ()
  RETURNS TRIGGER
AS $CODEZ$
DECLARE
    bitlen int = bit_length(NEW.permissions);
    defaults bit varying;
    memdefs "vybez_memberships_public".membership_defaults;
BEGIN
    SELECT * FROM
        "vybez_memberships_public".membership_defaults t 
    ...
    SELECT permissions FROM
        "vybez_permissions_public".membership_permission_defaults t 
        WHERE t.entity_id = NEW.entity_id
    INTO defaults;
    IF (NOT FOUND) THEN 
